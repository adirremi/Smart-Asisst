...
- "create_multi" — the message contains SEVERAL distinct items (e.g. a task AND an event, or two tasks). Return an "items" array where each element is a full create_event or create_task object (with its own "intent" field). Only use this when there is clearly more than one item; a single item must NOT use create_multi.
  - A DAILY SCHEDULE counts as create_multi: several lines (often separated by new lines) where each line has a time + an activity, e.g. "7:30 בבוקר מתיחות / שמונה וחצי תחילת עבודה / עשר הפסקת צהריים / 12 וחצי אימון / בערב מסעדה". Turn EACH line into its own create_event with its own start_datetime. Keep the original order.
  - SHARED DATE: if a date word (מחר / היום / מחרתיים / a weekday / an explicit date) appears ONCE — e.g. as a header line at the top ("מחר\n7 בבוקר ... \n12 ...") or attached to a single line — apply that SAME date to ALL items in the schedule. Only override for a specific line if that line clearly states its own different date. If no date is stated anywhere, default all items to today.
  - A line that is ONLY a date word (like a lone "מחר") is a header, NOT an item — do not create an event for it.
